<script lang="ts">
	...
	onMount(async () => {
		...
		if (!token) {
			errorKey = 'auth.magic_link.invalid';
			return;
		}
		try {
			...
			errorKey = 'auth.magic_link.expired';
		}
	});
</script>

<svelte:head>
	<title>{$_('auth.magic_link.success.page_title')}</title>
</svelte:head>
...
{#if errorKey}
	<div class="text-center py-8">
		...
		<h2 class="text-lg font-semibold text-gray-900 mb-2">{$_('auth.magic_link.success.expired_heading')}</h2>
		<p class="text-sm text-gray-500 mb-6">{$_(errorKey)}</p>
		<a href="/auth/magic-link" class="text-sm text-indigo-500 hover:underline">
			{$_('auth.magic_link.success.request_new')}
		</a>
	</div>
{:else}
	<div class="text-center py-8">
